...
interface NeuroWelcomeDashboardProps {
...
const TRENDING_TOPICS = [
  { tag: '#NeuroClarity', count: '1.2k' },
  { tag: '#ADHD_Focus', count: '850' },
  { tag: '#MarketPhysics', count: '2.4k' },
  { tag: '#ClearPath', count: '3.1k' },
];

const RECENT_ACTIVITY = [
  { user: 'Lisandro Matos', action: 'applied ADHD profile to XAU/USD', time: '2m ago', avatar: 'https://picsum.photos/seed/user1/100/100' },
  { user: 'Gvozden Boskovsky', action: 'shared a 15m BTC markup', time: '15m ago', avatar: 'https://picsum.photos/seed/user2/100/100' },
  { user: 'Hnek Fortuin', action: 'switched to Calm Focus mode', time: '1h ago', avatar: 'https://picsum.photos/seed/user3/100/100' },
];

export const NeuroWelcomeDashboard: React.FC<NeuroWelcomeDashboardProps> = ({ onNavigate }) => {
    ...
    <div className="space-y-12 pb-20 bg-black min-h-screen">
      ...
      <section className="relative h-[70vh] min-h-[500px] rounded-b-[60px] overflow-hidden flex items-center px-6 md:px-12 border-b border-white/5 shadow-2xl glass">
        <div className="absolute inset-0 z-0">
          <LightweightCandles profileId="adhd_dopamine_balanced" symbol="XAU/USD" timeframe="1h" />
        ...
        <div className="relative z-10 max-w-3xl space-y-10">
          <div className="flex items-center space-x-3 bg-indigo-500/10 border border-indigo-500/20 px-5 py-2 rounded-full w-fit backdrop-blur-md">
            ...
            <span className="text-[11px] font-black uppercase tracking-[0.3em] text-indigo-500">System Live // v4.2.0</span>
          </div>
          ...
          >
            NEURO <br />
            <span className="text-indigo-500">ADAPTIVE</span> <br />
            INSIGHTS
          </motion.h2>
          
          <p className="text-gray-400 font-mono text-sm md:text-base uppercase tracking-widest max-w-xl leading-relaxed">
            ClearPath Insights regulates interface <span className="neon-indigo-text">physics</span> to match your cognitive profile. 
            No advice. No trading. Just clarity.
          </p>

          <div className="flex items-center space-x-8 pt-6">
            <button 
              onClick={() => onNavigate?.('neurodivergent')}
            ...
            >
              <span className="lava-hot-text">Explore Profiles</span>
            </button>
            <div className="flex items-center space-x-4">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map(i => (
                  <img key={i} src={`https://picsum.photos/seed/u${i}/100/100`} className="w-12 h-12 rounded-full border-2 border-[#FF4500] object-cover shadow-[0_0_20px_#FF4500]" alt="User" />
                ))}
              </div>
          ...
          <section className="space-y-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <h3 className="text-xl font-black tracking-tighter uppercase italic">Neuro-Adaptive Showcase</h3>
              </div>
              <span className="text-[10px] font-mono uppercase tracking-widest opacity-50">4 of 20 Active Modes</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {SHOWCASE_PROFILES.map((pid) => {
                const p = NEURO_PROFILES[pid];
                return (
        ...
        <div className="space-y-12">
          ...
          <section className="glass border border-white/10 rounded-[32px] p-8 space-y-6">
            <div className="flex items-center space-x-3">
              ...
              <h4 className="text-sm font-black uppercase tracking-widest italic">Trending Insights</h4>
            </div>
            <div className="space-y-4">
              {TRENDING_TOPICS.map((topic) => (
                <div key={topic.tag} className="flex items-center justify-between group cursor-pointer">
                  ...
                  <span className="text-[10px] font-mono opacity-40">{topic.count} posts</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="space-y-6">
            <div className="flex items-center space-x-3">
              ...
              <h4 className="text-sm font-black uppercase tracking-widest italic">Recent Activity</h4>
            </div>
            <div className="space-y-6">
              {RECENT_ACTIVITY.map((act, i) => (
                <div key={i} className="flex items-start space-x-4">
                  <img src={act.avatar} className="w-8 h-8 rounded-full border-2 border-[#FF4500] object-cover shadow-[0_0_15px_#FF4500]" alt={act.user} />
