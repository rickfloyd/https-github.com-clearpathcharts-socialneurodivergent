...
export default function FxSikaJournal({ profile, onBackToDashboard }: FxSikaJournalProps) {
  ...
  // Load from Storage
  useEffect(() => {
    const savedTrades = localStorage.getItem('fxsika_trades');
    const savedSettings = localStorage.getItem('fxsika_settings');
  ...
  // Save to Storage
  useEffect(() => {
    localStorage.setItem('fxsika_trades', JSON.stringify(trades));
  }, [trades]);

  useEffect(() => {
    localStorage.setItem('fxsika_settings', JSON.stringify(settings));
  }, [settings]);
  ...
  // Stats Calculation
  const metrics = (() => {
    const sorted = [...filteredTrades].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      setTrades(prev => prev.map(t => t.id === editingId ? newTrade : t));
      setEditingId(null);
    } else {
  ...
  const getDayStats = (day: Date) => {
    ...
    const dayTrades = trades.filter(t => format(parseISO(t.createdAt), 'yyyy-MM-dd') === dayKey);
                ...
                <tbody className="divide-y divide-white/5">
                  {filteredTrades.slice().reverse().map(t => (
                    <tr key={t.id} className="hover:bg-white/5 transition-colors">
                      <td className="p-4 text-gray-500">{format(parseISO(t.createdAt), 'MM/dd')}</td>
